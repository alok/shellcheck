...
def getTokenChildren (t : Token) : List Token :=
  ...
  | .TA_Variable _ indices => indices
  | .TA_Trinary cond then_ else_ => [cond, then_, else_]
  | .TA_Parenthesis inner => [inner]
  | .TA_Assignment _ lhs rhs => [lhs, rhs]
  | .TA_Expansion parts => parts
  | .TA_Sequence exprs => exprs
