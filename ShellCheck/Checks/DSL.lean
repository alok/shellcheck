...
def sc2xxxMissing : List SCCode := [
  ...
  { code := 2295, severity := .warn, message := "Expansions inside ${..} don't accept quotes. Use temp var." },
  ...
  { code := 2302, severity := .warn, message := "Prefer $(...) over \\`...\\` for command substitution." },
  { code := 2303, severity := .warn, message := "This construct will be interpreted literally by echo." },
  ...
  { code := 2310, severity := .info, message := "This function is never invoked." },
  { code := 2311, severity := .info, message := "Bash does not support multidimensional arrays." },
