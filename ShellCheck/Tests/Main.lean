...
def regressionCases : List RegressionCase := [
  ...
  { name := "SC2096: shebang params warn", test := test_sc2096_shebang_params_warn },
  { name := "SC2096: shebang params ok", test := test_sc2096_shebang_params_ok },
  { name := "SC2096: shebang params env -S ok", test := test_sc2096_shebang_params_env_split_ok },
  { name := "SC2096: shebang params env --split-string ok", test := test_sc2096_shebang_params_env_split_string_ok },
  { name := "SC2148: missing shebang warn", test := test_sc2148_shebang_missing_warn },
  { name := "SC2148: shell override ok", test := test_sc2148_shebang_override_ok },
  { name := "SC2187: ash shebang warn", test := test_sc2187_shebang_ash_warn },
  { name := "SC2187: ash shebang override dash ok", test := test_sc2187_shebang_ash_override_dash_ok },
  { name := "SC2187: ash shebang override sh ok", test := test_sc2187_shebang_ash_override_sh_ok },
  { name := "SC2239: relative shebang warn", test := test_sc2239_shebang_relative_warn },
  { name := "SC2239: shebang shell override ok", test := test_sc2239_shebang_shell_override_ok },
  { name := "SC2239: shebang ignore ok", test := test_sc2239_shebang_ignore_ok },
  { name := "SC2246: shebang directory warn", test := test_sc2246_shebang_directory_warn },
  { name := "SC2246: shebang directory args warn", test := test_sc2246_shebang_directory_args_warn },
  { name := "SC2239: busybox shebang ok", test := test_sc2239_shebang_busybox_ok },
  { name := "SC2239: busybox shebang override sh ok", test := test_sc2239_shebang_busybox_shell_override_sh_ok },
  { name := "SC2239: busybox shebang override dash ok", test := test_sc2239_shebang_busybox_shell_override_dash_ok },
  { name := "SC2239: busybox ash shebang ok", test := test_sc2239_shebang_busybox_ash_ok },
  { name := "SC2239: busybox ash override dash ok", test := test_sc2239_shebang_busybox_ash_override_dash_ok },
  { name := "SC2239: busybox ash override sh ok", test := test_sc2239_shebang_busybox_ash_override_sh_ok },
  { name := "SC2013: for-in cat $(..)", test := test_sc2013_for_in_cat_subst },
  { name := "SC2013: for-in cat `..`", test := test_sc2013_for_in_cat_backticks },
  { name := "SC2013: for-in cat|grep", test := test_sc2013_for_in_cat_pipe_warn },
  { name := "SC2013: for-in cat|grep `..`", test := test_sc2013_for_in_cat_pipe_backticks_warn },
  { name := "SC2013: for-in cat|grep|wc ok", test := test_sc2013_for_in_cat_pipe_wc_ok },
  { name := "SC2036: assignment in pipeline", test := test_sc2036_assign_pipeline_warn },
  { name := "SC2036: assignment with command ok", test := test_sc2036_assign_pipeline_ok },
  { name := "SC2036: assignment only ok", test := test_sc2036_assign_only_ok },
  { name := "SC2037: assignment flag arg", test := test_sc2037_assign_command_flag },
  { name := "SC2037: assignment long flag arg", test := test_sc2037_assign_command_long_flag },
  { name := "SC2209: assignment command pipeline", test := test_sc2209_assign_command_pipeline },
  { name := "SC2209: assignment command ok", test := test_sc2209_assign_command_ok },
  { name := "SC2209: PAGER=cat warn", test := test_sc2209_pager_cat_warn },
  { name := "SC2209: PAGER=\"cat\" ok", test := test_sc2209_pager_quoted_ok },
  { name := "SC2209: assignment-only warn", test := test_sc2209_assign_only_warn },
  ...
  { name := "SC2066: for-in quoted subst", test := test_sc2066_for_in_quoted_subst },
  { name := "SC2066: for-in $@ ok", test := test_sc2066_for_in_dollar_at_ok },
  { name := "SC2043: for-in glob ok", test := test_sc2043_for_in_glob_ok },
  { name := "SC2066: for-in quoted glob", test := test_sc2066_for_in_quoted_glob },
  { name := "SC2041: for-in single-quoted literal", test := test_sc2041_for_in_single_quoted_literal },
  { name := "SC2042: for-in commas", test := test_sc2042_for_in_commas },
  { name := "SC2043: for-in brace ok", test := test_sc2043_for_in_brace_ok },
  { name := "SC2043: for-in single word once", test := test_sc2043_for_in_single_word_once },
  { name := "SC2043: for-in indirect ok", test := test_sc2043_for_in_indirect_ok },
  { name := "SC2258: for-in trailing comma", test := test_sc2258_for_in_trailing_comma },
  { name := "SC2258: for-in trailing comma quoted", test := test_sc2258_for_in_trailing_comma_quoted },
  { name := "SC2258: for-in comma quoted ok", test := test_sc2258_for_in_comma_inside_quotes_ok },
  { name := "SC2045: for-in ls $(..)", test := test_sc2045_for_in_ls_subst },
  { name := "SC2045: for-in ls `..`", test := test_sc2045_for_in_ls_backticks },
  { name := "SC2044: for-in find `..`", test := test_sc2044_for_in_find_backticks },
  { name := "SC2067: find -exec missing terminator", test := test_sc2067_find_exec_missing_semicolon },
  { name := "SC2067: find -exec uses &&", test := test_sc2067_find_exec_invalid_and },
  { name := "SC2067: find -exec pipeline", test := test_sc2067_find_exec_pipe },
  { name := "SC2067: find -exec ok", test := test_sc2067_find_exec_ok },
  { name := "SC2067: find -exec ok with shell", test := test_sc2067_find_exec_ok_with_shell },
  { name := "SC2014: find -exec glob warn", test := test_sc2014_find_exec_glob_warn },
  ...
  { name := "SC2023: time external warn", test := test_sc2023_time_external_warn },
  { name := "SC2023: time -p ok", test := test_sc2023_time_p_ok },
  { name := "SC2023: time non-bash ok", test := test_sc2023_time_non_bash_ok },
  ...
  { name := "SC2046: unquoted $(ls)", test := test_sc2046_unquoted_expansions_basic },
  { name := "SC2046: unquoted `ls`", test := test_sc2046_unquoted_expansions_backticks },
  { name := "SC2046: unquoted concat", test := test_sc2046_unquoted_expansions_concat },
  { name := "SC2046: unquoted test", test := test_sc2046_unquoted_expansions_test },
  { name := "SC2046: unquoted test negated", test := test_sc2046_unquoted_expansions_test_negated },
  { name := "SC2046: double bracket ok", test := test_sc2046_unquoted_expansions_double_bracket_ok },
  { name := "SC2046: for-in ok", test := test_sc2046_unquoted_expansions_for_in_ok },
  { name := "SC2046: command name ok", test := test_sc2046_unquoted_expansions_as_command_ok },
  { name := "SC2046: heredoc ok", test := test_sc2046_unquoted_expansions_heredoc_ok },
  { name := "SC2046: set -- ok", test := test_sc2046_unquoted_expansions_set_ok },
  { name := "SC2046: inline comment ok", test := test_sc2046_unquoted_expansions_comment_ok },
  { name := "SC2046: export warn", test := test_sc2046_unquoted_expansions_export_warn },
  { name := "SC2046: ps -p $(pgrep) ok", test := test_sc2046_unquoted_expansions_ps_pgrep_ok },
  { name := "SC2098: prefix assignment reference", test := test_sc2098_prefix_assignment_reference },
  { name := "SC2099: arith op simple", test := test_sc2099_arith_op_warn_simple },
  { name := "SC2099: arith op assign", test := test_sc2099_arith_op_warn_assign },
  { name := "SC2099: arith op ok", test := test_sc2099_arith_op_ok },
  { name := "SC2100: wrong arithmetic", test := test_sc2100_wrong_arith },
  { name := "SC2100: wrong arithmetic reference", test := test_sc2100_wrong_arith_reference },
  ...
  { name := "SC2111: ksh function keyword parens", test := test_sc2111_ksh_function_keyword_parens },
  { name := "SC2112: sh function keyword parens", test := test_sc2112_sh_function_keyword_parens },
  { name := "SC2113: sh function keyword no parens", test := test_sc2113_sh_function_keyword_no_parens },
  { name := "SC2112/2113: sh function parens only ok", test := test_sc2112_sh_function_parens_only_ok },
