...
def main : IO UInt32 := do
  ...
  ok := ok && (← checkProp "brace expansion splits alternatives" prop_brace_expansion_splits cfg)
  ok := ok && (← checkProp "brace expansion nested braces stay literal" prop_brace_expansion_nested_literal cfg)
  ok := ok && (← checkProp "brace expansion range stays expansion" prop_brace_expansion_range cfg)
  ok := ok && (← checkProp "extglob splits alternatives" prop_extglob_splits cfg)
  ok := ok && (← checkProp "extglob keeps | in bracket class" prop_extglob_bracket_pipe cfg)
  ok := ok && (← checkProp "heredoc unquoted expands" prop_heredoc_unquoted_expands cfg)
